...
Behind the card sit three fixed, full-viewport, `pointer-events: none` elements: `#loomBackdrop` (the page background and static dot grid, `z-index: -2`), and at `z-index: -1`, `#threadCanvas` (Loom E — a canvas engine that continuously draws faint themed threads between grid dots, holds them, then fades them) and `#dotWave` (Loom F — a one-shot brighter-dot sweep that plays when the user arrives at the dashboard from a wizard step). Both engines are purely decorative: they never intercept input, are skipped entirely under `prefers-reduced-motion`, and Loom E's render loop sleeps whenever nothing is animating. Static invariants for both are enforced by `scripts/local-verify.js` (`checkLoomEF`); visual behavior is verified manually per the [QA runbook](qa-runbook.md).
