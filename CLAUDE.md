...
Single-file Google Apps Script webapp. All HTML, CSS, and JS live in `Index.html` (~2200 lines). Server-side logic is in `Code.js`. There is no build step — deploy via `clasp push` or GitHub Actions.
...
- **z-index hierarchy**: `#statusToast` = 11000, `#debugCopiedToast` = 10500, `_showFallback` overlay = 9999. Don't break this stack.

## Testing

No automated test framework. All testing is manual. When adding features:
- Run the QA checklist manually (see test plan in `~/.gstack/projects/DaftVino-SheetWeaver-Webapp/`)
